...
jQuery(document).ready(function(a) {
	...
	}), a(".user-signin, .user-reg").on("click", function(c) {
		a("div.overlay").length <= 0 ? a("#header").before('<div class="overlay"></div>') : a("div.overlay").show(), a("body").addClass("fadeIn"), 1 == a(this).attr("data-sign") ? b.removeClass("sign").addClass("register") : b.removeClass("register").addClass("sign"), a("div.overlay, form a.close").on("click", function() {
...
function Doreg(){
	...
	var user_yzm = $("input[name='user_yzm']").val();
	var vdcode_reg = $("input[name='vdcode_reg']").val();
	var qmold = $("input[name='qmold']").val();
	
	var errmsg = $("<label class='error'></label>");
	var zyw = /^(([a-zA-Z]))[a-zA-Z]*$/;
	var emailyz = /\w+[@]{1}\w+[.]\w+/;
	...
	if(user_name==''){
		...
		return false
	}else if(user_pass==user_name){
		$(".error").html("");
		errmsg.html("密码不能和登录名相同").attr("id","password-error").appendTo(".user_pass");
		return false
		...
		return false
	}else if(!emailyz.test(user_email)){
		$(".error").html("");
		errmsg.html("邮箱格式不正确").attr("id","email-error").appendTo(".user_email");
		return false
	}else if(user_yzm==''){
		$(".error").html("");
		errmsg.html("请填写邮箱验证码").attr("id","user_yzm-error").appendTo(".user_yzm");
		return false
	}else if(user_yzm.length < 4){
		$(".error").html("");
		errmsg.html("邮箱验证码为4位").attr("id","user_yzm-error").appendTo(".user_yzm");
		return false
	}else if(vdcode_reg==''){
		$(".error").html("");
		errmsg.html("请输入"+$("#vdcode").attr("placeholder")).attr("id","vdcode-error").css({right:'95px'}).appendTo(".vdcode_reg");
		return false
	}else if(vdcode_reg.length < 4){
		$(".error").html("");
		errmsg.html("验证码长度不足4位").attr("id","vdcode-error").css({right:'95px'}).appendTo(".vdcode_reg");
		return false
	}else{
		$(".error").html("");
		$.ajax({
...
$(document).ready(function(){
	...
	$("#user_email").change(function(){
		var errmsg = $("<label class='error'></label>");
		var emailyz = /\w+[@]{1}\w+[.]\w+/;
		var oldemail = $("#user_email").val();
		var uname = $("#user_name").val();
		if(oldemail==''){
			errmsg.html("请"+$("#user_email").attr("placeholder")).attr("id","email-error").appendTo(".user_email");
			return false
		}else if(!emailyz.test(oldemail)){
			errmsg.html("邮箱格式不正确").attr("id","email-error").appendTo(".user_email");
			return false
		}else{
			$("#email-error").html("");
			$(".user_email label").css({right:"100px"});
			$(this).css({width:"73%"});
			$("#send_email").html("发送验证码").attr("onClick","Send_email('"+oldemail+"','"+uname+"')").attr("class","vdcodeimg").css({"display":"inline-block","padding":"0","vertical-align":"middle","width":"80px","height":"38px","line-height":"38px","color":"#333","text-align":"center"});
			$(".user_yzm").show();
		}
	});
});
...
//发送邮件验证码
function Send_email(email,uname){
	$.ajax({
		type: 'post',
		url: '/i/vdcode_sms.php',
		data:{
			dopost: 'send',
			uemail: email,
			uname: uname
		...
		beforeSend: function () {
			// 禁用按钮防止重复提交
			$("#send_email").html("发送中");
			$("#send_email").attr("data-click",$("#send_email").attr("onclick"));
			$("#send_email").removeAttr("onclick");
		},
		success: function(data) {
			if(data.son=="ok"){
				$("#send_email").html("发送成功");
				$(".user_yzm").show();
				$("#qmold").val(data.oldyzm);
				setTimeout(function(){
					$("#send_email").html("重新发送");
					$("#send_email").attr({"onclick":$("#send_email").attr("data-click")});
					$("#send_email").removeAttr("data-click");
				}, 120000)
			}else{
				$("#send_email").html("发送失败");
				setTimeout(function(){
					$("#send_email").html("重新发送");
					$("#send_email").attr({"onclick":$("#send_email").attr("data-click")});
					$("#send_email").removeAttr("data-click");
