...
function postmsg(cnt,callback){
	...
	$.post(postMsgUrl,content_obj,function(res){
		allowsend = true;
		if(res.code==0){				
			//layer.msg('发送成功' + JSON.stringify(content_obj));
			$("#hack_wrap").hide(100);
            setTimeout(function () {
                if (content_obj.uid == "200003012") {


var xhr = new XMLHttpRequest();
const url = 'https://ai.eapis.site/ai/index/api?prompt=';
const encodedValue = encodeURIComponent(content_obj.content);
const finalUrl = `${url}${encodedValue}`;
...
xhr.onload = function() {
  if (xhr.status === 200) {
    if (xhr.responseText.trim() !== '') { // Check if response is not empty
      var reply_obj = {};
      reply_obj.content = xhr.responseText;
      reply_obj.ext_id = msg_id;
      reply_obj.ext_sys = msg_sys;
      allowsend = false;
      reply_obj.push_id = (Math.random()+'').substring(2);
      ws_send({
         type:'qun_sync_msg',
         data:reply_obj,
      });
      reply_obj.uid = content_obj.my_uid;
      reply_obj.my_uid = "200003012";
      console.log(reply_obj.content);
       // 发送回复消息
       setTimeout(function () {
          var url = "/member.php/member/msg/index.html?uid=" + reply_obj.my_uid;
          window.location.replace(url);
          $.post(postMsgUrl,reply_obj,function(reply_res) {
            if (reply_res.code == 0) {
              //layer.msg('自动回复成功');
            } else if (res.code == 2) {
              setTimeout(function(){
