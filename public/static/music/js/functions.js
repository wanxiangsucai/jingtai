...
$(function(){
    ...
    // 分享歌曲到聊天框
    $('.music-list').on('click','.share-to-chat',function(){
        var num = parseInt($(this).parent().data("no"));
        sharetochat(rem.dislist, num);
        return false;
    });
    // 列表鼠标移过显示对应的操作按钮
    $(".music-list").on("mousemove",".list-item", function() {
        ...
        if(!$(this).data("loadmenu")) {
            ...
            var html = '<span class="music-name-cult" data-no="' + num + '">' +
            target.html() + 
            ' <a href="jascript:;" class="share-to-chat" style="border:1px solid #eeeeee;color: #fff;font-size: 12px;">分享</a></span>' +
    ...
    // 图片加载失败处理
    $('img').error(function(){
        $(this).attr('src', window.location.protocol+"//"+window.location.host+'/public/static/music/images/player_cover.png');
    });
...
// 分享到聊天框chat
function sharetochat(list, index){
    thisShareToChat(list,index);
}
...
function thisShare(obj) {
    ...
    ajaxUrl(musicList[$(obj).data("list")].item[$(obj).data("index")], AgainShareToChat);
}
// 分享到聊天框步骤二
function thisShareToChat(list,index){
    ajaxUrl(musicList[list].item[index],AgainShareToChat);
}
// 分享到聊天框步骤三
function AgainShareToChat(music){
    if(music.url == 'err' || music.url == "" || music.url == null) {
        layer.msg('这首歌不支持外链获取',{icon:5,anim:6});
        return;
    }
    var content = '分享音乐&nbsp;&nbsp;'+music.name+'<br><audio src="' + music.url + '" name="'+music.name+'" controls preload="auto"></audio>';
	
    if(rem.isMobile) {
		parent.$('textarea').val(content);
		parent.$("#btnSend").click();
		var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
		parent.layer.close(index);
	}else{
		parent.$('#input_box').val(content);
		parent.$("#send").click();
	}
...
function changeCover(music) {
    ...
    if(img == "err") {
        img = window.location.protocol+"//"+window.location.host+"/public/static/music/images/player_cover.png";
    } else {
...
function addSheet(no, name, cover) {
    if(!cover) cover = window.location.protocol+"//"+window.location.host+"/public/static/music/images/player_cover.png";
    if(!name) name = "读取中...";
...
function clearDislist() {
    ...
    if(rem.dislist == 1) {  // 正在播放列表
        ...
        $(".sheet-item[data-no='1'] .sheet-cover").attr('src', 'http://'+window.location.host+'/public/static/music/images/player_cover.png');    // 恢复正在播放的封面
